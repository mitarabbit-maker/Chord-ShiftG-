...
function transposeChordText(text, semitones, useFlat) {
  return text.replace(CHORD_ROOT_RE, (_, root, suffix) =>
    transposeNote(root, semitones, useFlat) + suffix
  );
}
function getCapoTable(targetKey) {
  return Array.from({ length: 8 }, (_, fret) => ({
    fret,
...
const T = {
  ...
  en: {
    ...
    revDesc:'Select capo position and chord shape - see what key is actually sounding.',
    revCapoPos:'Capo Position', revCapoNone:'No capo (open)',
    revCapoFret:(n)=>`Fret ${n}`,
    revRoot:'Root note (shape you are pressing)', revQuality:'Chord Quality',
    ...
    revDiagPlayed:'Shape you are playing', revDiagSounding:'Actual sounding chord',
      ...
      <div style={{textAlign:'center',width:96}}>
        ...
        <div style={{width:70,height:88,margin:'0 auto',border:'1px dashed #2e2a25',
          ...
          <span style={{fontSize:10,color:'#555',fontFamily:'DM Mono,monospace',
            textAlign:'center',lineHeight:1.6}}>No diagram</span>
        </div>
      </div>
    );
  }
...
document.head.appendChild(link);
    ...
    setMeta('description', 'カポ位置とコードフォームを選ぶだけで実際に鳴っているキーがわかる。ギタリストのための無料コード変換ツール。');
              ...
              <div className="key-grid">
                {KEYS.map(k=>(
                  <button key={k}
                    ...
                    onClick={()=>setRevShape(k)}>{k}</button>
                ))}
              </div>
            </div>
            <div className="field">
            ...
            <div className="capo-table" style={{marginTop:10}}>
              {capoTable.map(({fret,shape})=>(
                <div key={fret} className={`capo-cell ${fret===0?'open':''}`}>
                  <div className="cc-fret">{fret===0?t.capoNone:t.capoFret(fret)}</div>
